<template>
  <div class="mx-auto w-full max-w-6xl px-5 py-10 sm:px-8 lg:py-16">
    ...
    <div v-else class="grid gap-8 lg:grid-cols-[.75fr_1.25fr] lg:items-start">
      <aside><span class="tool-icon tool-icon--large"><component :is="tool.icon" :size="26" /></span><p class="section-kicker mt-6">{{ tool.category }} tool</p><h1 class="mt-2 text-4xl font-bold tracking-[-0.04em] text-slate-950">{{ tool.name }}</h1><p class="mt-4 leading-7 text-slate-500">{{ tool.description }}</p><div class="mt-7 grid grid-cols-2 gap-3"><div class="meta-card"><span>Input</span><strong>{{ tool.input }}</strong></div><div class="meta-card"><span>Output</span><strong>{{ tool.output }}</strong></div></div><div class="mt-7 flex items-start gap-3 rounded-2xl bg-blue-50/70 p-4 text-sm leading-6 text-slate-600"><ShieldCheck :size="18" class="mt-0.5 shrink-0 text-cobalt" /><p>Files are processed for this job only. This MVP does not persist user history yet.</p></div></aside>
